...
class  GeofireProvider {
  ...
  Stream<List<DocumentSnapshot>> getNearbyMusics(double lat, double lng, double radius) {
    GeoFirePoint center = _geo.point(latitude: lat, longitude: lng);
    return _geo.collection(
        collectionRef: _ref.where('status', isEqualTo: 'musics_available')
  ...
  Future<void> create(String id, double lat, double lng) {
    ...
    return _ref.doc(id).set({'status': 'musics_available', 'position': myLocation.data});
  }
